...
/**
 * Simple template engine functions that replaces ${...} occurrences in an array of strings or a single string.
...
public class BasicStringReplace {
	 ...
	 * @param envMap map of replacement strings
	 ...
	 */
	public static String untemplate(String[] strArray, Map<String, String> envMap, boolean bFailOnMissing, String separator) {
		StringBuilder sb = new StringBuilder();
		if (strArray != null) {
			for (int i = 0; i < strArray.length; ++i) {
				sb.append(untemplate(strArray[i], envMap, bFailOnMissing));
				if (separator != null) {
					sb.append(separator);
				}
			}
	 ...
	 */
	public static String untemplate(String str, Map<String, String> envMap, boolean bFailOnMissing) {
				...
				if (sIdx + 1 < strLen) {
					...
					if (c == '$') {
						...
						if (tIdx != -1) {
							...
							replaceStr = envMap.get(lookupStr);
							if (replaceStr == null) {
								if (!bFailOnMissing) {
									replaceStr = "";
								}
